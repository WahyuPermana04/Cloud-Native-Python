...
class FlaskappTests(unittest.TestCase):
	...
	def test_addtweets_status_code(self):
		# sends HTTP GET request to the application
		# on the specified path
		result = self.app.post('/api/v2/tweets', data='{"username":"dummy", "body":"Wow! Is it working testing"}', content_type='application/json')
		# assert the status code of the response
	...
	def test_delusers_status_code(self):
		...
		"cobagain"}', content_type='application/json')
		# assert the status code of the response
		self.assertEquals(result.status_code, 200)	
